locals {
  ...
  task_definition = local.create_task_definition ? join(":", [
    join("", aws_ecs_task_definition.default[*].family),
    var.task_definition_family_only ? "" : join("", aws_ecs_task_definition.default[*].revision)
    ]
  ) : var.task_definition[0]
}
