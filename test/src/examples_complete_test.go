...
func TestExamplesComplete(t *testing.T) {
	...
	// Verify we're getting back the outputs we expect
	assert.Equal(t, "arn:aws:ecs:us-east-2:"+accountID+":cluster/"+basename, ecsClusterId)

	// Run `terraform output` to get the value of an output variable
	ecsClusterArn := terraform.Output(t, terraformOptions, "ecs_cluster_arn")
	// Verify we're getting back the outputs we expect
	assert.Equal(t, "arn:aws:ecs:us-east-2:"+accountID+":cluster/"+basename, ecsClusterArn)
	...
	// Verify we're getting back the outputs we expect
	assert.Equal(t, "arn:aws:iam::"+accountID+":role/"+basename+"-exec", taskExecRoleArn)
	...
	// Verify we're getting back the outputs we expect
	assert.Equal(t, "arn:aws:iam::"+accountID+":role/"+basename+"-task", taskRoleArn)
}
